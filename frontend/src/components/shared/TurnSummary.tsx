...
export default function TurnSummary({
  ...
  return (
    <div
      className={`fixed inset-0 flex flex-col items-center justify-center bg-base/90 z-50 ${isLeaving ? "animate-overlay-out" : "animate-overlay-in"}`}
    >
      <div className="animate-pop-in flex flex-col items-center gap-6 bg-surface border border-accent rounded-2xl px-12 py-10">
        {isBust ? (
          ...
          <img src={washingMachineImg} alt="Waschmaschine" className="w-max" />
        ) : (
